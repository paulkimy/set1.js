...
function savings(grossPay, taxRate, expenses) {
    ...
    let takeHomePay = afterTaxPay - expenses;
    return takeHomePay;
  }
...
function materialWaste(totalMaterial, materialUnits, numJobs, jobConsumption) {
    let totalConsumption = numJobs * jobConsumption;
    let remainingMaterial = totalMaterial - totalConsumption;
...
function interest(principal, rate, periods) {
    let interest = principal * rate * periods;
    let finalValue = principal + interest;
